...
for variable=1:3
    for scenario=1:nScenarios
        ...
        for iter=[1,11,39,59] %0:length(time)-1
            ...
            if variable ==1
                ...
                hold on
                caxis([0 15]) %pressure
                disp(['maxP=',num2str(max(abs(PP(:)-PP0(:))/1e6))])
            elseif variable ==2
                [c,h]=contourf(XX,ZZ,(TT-TT0));
                hold on
                caxis([-15 190]) %temperature
                disp(['maxT=',num2str(max(abs(TT(:)-TT0(:))))])
            elseif variable ==3
            ...
            if nplot==1
                text(4000,-500,['\color{white} \fontsize{14} Unrest x',Scenario_letter{scenario}]);
                if scenario==3
                    %             title(['pore pressure changes P-P_0 [MPa],   time: ',num2str(round(time_*100)/100),' years'],'FontSize',16)
                    if variable ==1
                        title(['Pore pressure changes P-P_0 [MPa]'],'FontSize',FontSize2)
                    elseif variable ==2
                        title(['Temperature changes T-T_0 [C]'],'FontSize',FontSize2)
                    elseif variable ==3
                        title(['Gas saturation changes S_g-S_{g0}'],'FontSize',FontSize2)
                    end
    ...
    if variable ==1
        ...
        hold on
%         caxis([0 4]) %pressure
        title(['Initial condition: pore pressure changes P_0 [MPa]'],'FontSize',FontSize2)
    elseif variable ==2
        [c,h]=contourf(XX,ZZ,(TT0));
        hold on
%         caxis([-15 170]) %temperature
        title(['Initial condition: temperature changes T_0 [C]'],'FontSize',FontSize2)
        set(gca,'yticklabel',[])
    elseif variable ==3
        [c,h]=contourf(XX,ZZ,(SSGG0));
        hold on
        caxis([0 1]) %gas saturation
        title(['Initial condition: gas saturation changes S_{g0}'],'FontSize',FontSize2)
        set(gca,'yticklabel',[])
    end
                plot([Fault1P1(1),Fault1P2(1)],[Fault1P1(2),Fault1P2(2)],'-w','lineWidth',1.2)
            plot([Fault2P1(1),Fault2P2(1)],[Fault2P1(2),Fault2P2(2)],'-w','lineWidth',1.2)
