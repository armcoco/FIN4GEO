function [u,v] = Jacobi(u,v,dx,dy,Inside,Ghost,Mask,F,G,NU,BC,Xt1inv,Yt1inv)
...
for iter=1:overRel
    ...
    for k=Inside.B';
        ...
        uu=u0(stencil_uu(:))'*coeffs_uu(:)+v0(stencil_uv(:))'*coeffs_uv(:);
        vv=u0(stencil_vu(:))'*coeffs_vu(:)+v0(stencil_vv(:))'*coeffs_vv(:);
        dtu=1/(coeffs_uu(1));
        dtv=1/(coeffs_vv(1));
    ...
    for current=Ghost.Bdy
        k = current.index;
        type=current.type;
        uu=u0(current.stencil(:))'*current.coeffsD(:);
        vv=v0(current.stencil(:))'*current.coeffsD(:);
        dtu=dtD;
        dtv=dtD;
        u(k)=u(k)+dtu*(F.u(k)-uu);
        v(k)=v(k)+dtv*(F.v(k)-vv);
...
%     coeffs_uu=nx*coeffs_dx(:)+ny*coeffs_dy(:);
%     coeffs_vv=nx*coeffs_dx(:)+ny*coeffs_dy(:);
%     coeffs_uv=zeros(9,1);
%     coeffs_vu=zeros(9,1);
    
    uu=u0(stencil_uu(:))'*coeffs_uu(:)+v0(stencil_uv(:))'*coeffs_uv(:);
    vv=u0(stencil_vu(:))'*coeffs_vu(:)+v0(stencil_vv(:))'*coeffs_vv(:);
    dtu=1*dtN/g/max(current.xt1inv,current.yt1inv);
    dtv=1*dtN/g/max(current.xt1inv,current.yt1inv);
    u(k)=u(k)+dtu*(F.u(k)-uu);
    v(k)=v(k)+dtv*(F.v(k)-vv);
end
