...
% % %     caxis([0 2.5])
    %     xlim([0 1000])
    colorbar
    title(['pore pressure P_0 [MPa],   time: ',num2str(round(time_*100)/100),' years'],'FontSize',16)
...
% % %     caxis([0 5])
    %     xlim([0 1000])
    colorbar
    title('temperature T_0 [C]','FontSize',16)
    hold on
%     plottaGradT;
...
%     title('density changes -(\rho-\rho_0) [Kg/m^3]','FontSize',16)
    title('gas saturation S_{g0}','FontSize',16)
