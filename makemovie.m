...
%         xlim([0 10000])
    colorbar
    title(['temperature at time=0'],'FontSize',16)
...
%         xlim([0 3000])
    colorbar
    title('temperature change: T-T_0[C]','FontSize',16)
    hold on
    %     plottaGradT;
...
%     plot(ttt',V_axis_onlyP,'--r','lineWidth',2);
    ...
    plot(ttt',V_axis_new-V_axis_onlyP_new,'-b','lineWidth',2)
    hold on
    plot(tt(iter+1)',V_axis(iter+1)-V_axis_onlyP(iter+1),'or','lineWidth',2)
    ylim([-0.02 0.07])
    
    drawnow;
    disp(['time: ',num2str(time(iter+1)/86400/365.25)])
%     pause;
    mov(iter+1)=getframe(gcf);
end
